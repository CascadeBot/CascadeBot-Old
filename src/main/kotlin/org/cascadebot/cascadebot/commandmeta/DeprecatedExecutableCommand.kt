...
@Deprecated("Old Command System", ReplaceWith("ExecutableRootCommand"))
abstract class DeprecatedExecutableCommand {
    ...
    val absoluteCommand: String
        get() = when (this) {
            is DeprecatedSubCommand -> this.parent() + "." + command()
            else -> command()
        }
    ...
    fun fullCommand(locale: Locale): String {
        if (this is DeprecatedSubCommand) {
