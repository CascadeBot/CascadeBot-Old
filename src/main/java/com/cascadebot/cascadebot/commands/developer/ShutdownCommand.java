...
public class ShutdownCommand implements ICommandRestricted {
    ...
    public void onCommand(Member sender, CommandContext context) {
        ...
        if (!CascadeBot.getVersion().getBuild().equalsIgnoreCase("dev") && !ConfirmUtils.hasConfirmedAction(sender.getUser().getIdLong(), "shutdown_bot")) {
            context.replyDanger("It looks like the bot is running in production mode, **do you really want to do this?** \nIf so, simply repeat the command again. This confirmation will expire in one minute!");
            ConfirmUtils.confirmAction(sender.getUser().getIdLong(), "shutdown_bot");
            return;
        }
