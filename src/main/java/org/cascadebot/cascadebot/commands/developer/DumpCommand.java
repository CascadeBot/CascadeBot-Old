...
public class DumpCommand implements ICommandRestricted {
    ...
    public void onCommand(Member sender, CommandContext context) {
        if (context.getArgs().length < 1) {
            context.getTypedMessaging().replyDanger("commands.developer.dump.not_enough_arguments");
            return;
        }
        if (context.getArg(0).equalsIgnoreCase("threads")) {
            ...
            Table.TableBuilder builder = new Table.TableBuilder(context.i18n("commands.developer.dump.command"), context.i18n("commands.developer.dump.module"), context.i18n("commands.developer.dump.permission"), context.i18n("commands.developer.dump.subcommand"));
            for (ICommandMain command : CascadeBot.INS.getCommandManager().getCommands()) {
                builder.addRow(
                    command.command(),
                    command.getModule().toString(),
                    command.getPermission() == null ? context.i18n("commands.developer.dump.no_permission") : command.getPermission().toString(),
            ...
            Table.TableBuilder builder = new Table.TableBuilder(context.i18n("commands.developer.dump.permission"), context.i18n("commands.developer.dump.discord_perms"), context.i18n("commands.developer.dump.default_permission"));
            ...
            context.getTypedMessaging().replyDanger(context.i18n("commands.developer.dump.cannot_find_argument", UnicodeConstants.THINKING));
        }
    }
