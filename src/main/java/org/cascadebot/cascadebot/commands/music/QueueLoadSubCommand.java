...
public class QueueLoadSubCommand implements ISubCommand {
    ...
    public void onCommand(Member sender, CommandContext context) {
        ...
        context.getMusicPlayer().loadPlaylist(context.getArg(0), sender, context.getChannel().getIdLong(), context.getGuild().getIdLong(), (result, tracks) -> {
            switch (result) {
                case LOADED_GUILD:
                case LOADED_USER:
                ...
                case EXISTS_IN_ALL_SCOPES:
                    ...
                    buttonGroup.addButton(new Button.UnicodeButton(UnicodeConstants.ONE, ((runner, channel, message) -> {
                        ...
                        context.getMusicPlayer().loadPlaylist(context.getArg(0), sender, PlaylistType.USER, context.getChannel().getIdLong(), context.getGuild().getIdLong(), ((loadPlaylistResult, newTracks) -> {
                    ...
                    buttonGroup.addButton(new Button.UnicodeButton(UnicodeConstants.TWO, ((runner, channel, message) -> {
                        ...
                        message.delete().queue(null, DiscordUtils.handleExpectedErrors(ErrorResponse.UNKNOWN_MESSAGE));
                        context.getMusicPlayer().loadPlaylist(context.getArg(0), sender, PlaylistType.GUILD, context.getChannel().getIdLong(), context.getGuild().getIdLong(), ((loadPlaylistResult, newTracks) -> {
                            context.getUIMessaging().sendTracksFound(newTracks);
                        }));
                    })));
