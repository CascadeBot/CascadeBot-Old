...
public class ModlogChannelSubCommand extends SubCommand {
    ...
    public void onCommand(Member sender, CommandContext context) {
        ...
        String channelInput = context.getArg(0);
        String operation = context.getArg(1);
        TextChannel channel = DiscordUtils.getTextChannel(context.getGuild(), channelInput);
        if (channel == null) {
            context.getTypedMessaging().replyDanger("Couldn't find channel `" + channelInput + "`");
            return;
        }
        ...
        if (operation.equalsIgnoreCase("enable")) {
            channelEnable(context, channel);
            return;
        } else if (operation.equalsIgnoreCase("disable")) {
            channelDisable(context, channel);
            return;
        } else if (operation.equalsIgnoreCase("delete")) {
            channelDelete(context, channel);
            return;
    ...
    private void channelMove(CommandContext context) {
    ...
    private void channelList(CommandContext context) {
    ...
    private void channelInfo(CommandContext context, TextChannel channel) {
    ...
    private void channelDelete(CommandContext context, TextChannel channel) {
        context.getData().write(guildData -> {
            guildData.getModeration().removeModlogEvent(String.valueOf(channel.getIdLong()));
        });
        context.getTypedMessaging().replySuccess("Successfully deleted channel!");
    }

    private void channelDisable(CommandContext context, TextChannel channel) {
    ...
    private void channelEnable(CommandContext context, TextChannel channel) {
        if (isEnabled(channel, context)) {
